...
def method_relaxation(matrix, w, e, name_matrix, name_unknown):
    ...
    if (np.array(matrix).transpose() == np.array(matrix)).all() and is_positive_definite_matrix(matrix) and np.all(np.linalg.eigvals(matrix) > 0) and 0 < w < 2:
...
def method_rapid_descent(matrix, e, name_matrix, name_unknown):
    ...
    if not is_positive_definite_matrix(matrix):
        return
